...
class MonteCarloSuite extends munit.FunSuite:
  ...
  test("replicate streams split deterministically by id") {
    val seed = Seed.fromLong(1729L)
    val zero = accepted(ReplicateId(0))
    val one = accepted(ReplicateId(1))
    val p0a = accepted(ResamplingPlans.permutation(seed, zero, 12))
    val p0b = accepted(ResamplingPlans.permutation(seed, zero, 12))
    val p1 = accepted(ResamplingPlans.permutation(seed, one, 12))
    assertEquals(p0a, p0b)
    assertNotEquals(p0a, p1)
    assertEquals(p0a.toVector.sorted, Vector.range(0, 12))
    assertEquals(p1.toVector.sorted, Vector.range(0, 12))
  }
