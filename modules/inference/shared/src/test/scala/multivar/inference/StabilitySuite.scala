...
class StabilitySuite extends munit.FunSuite:
  ...
  test("row and cluster bootstrap actions are deterministic replicate programs") {
    ...
    val first = accepted(rowAction.draw(Seed.fromLong(19L), replicate))
    val second = accepted(rowAction.draw(Seed.fromLong(19L), replicate))
    assertEquals(first, second)
    assertEquals(first.domain, 6)
    assert(first.toVector.forall(row => row >= 0 && row < 6))

    val partition = accepted(ClusterPartition.from(
      accepted(RowCount(6)),
      Vector(Vector(0, 1), Vector(2, 3), Vector(4, 5))
    ))
    val clustered = accepted(BootstrapAction
      .clusters(partition)
      .draw(Seed.fromLong(23L), replicate))
    assertEquals(clustered.domain, 6)
    assert(clustered.toVector.grouped(2).forall { rows =>
      rows.length == 2 && rows(1) == rows(0) + 1 && rows(0) % 2 == 0
    })
  }
