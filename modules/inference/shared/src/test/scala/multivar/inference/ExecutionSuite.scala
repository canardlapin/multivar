...
class ExecutionSuite extends munit.FunSuite:
  ...
  private def acceptedResample[A](
  ...
  test("compiled programs lower to deterministic scheduler-neutral replicate jobs") {
    ...
    assertEquals(first.randomAlgorithm, ResamplingPlans.permutationAlgorithm)
    assertEquals(first.seedDerivation, Seed.derivationAlgorithm)
  ...
  test("thin-SVD exact updates match full paired refits at numerical precision") {
    ...
    val permutations = Vector(
      ...
      acceptedResample(Permutation.from(IArray.from(rows)))
    )

    permutations.foreach { permutation =>
      val full = accepted(RefitExecutor.full(data, permutation, Seed.fromLong(71L)))
      val exact = accepted(RefitExecutor.exact(data, observed, permutation, Seed.fromLong(71L)))
      ...
      exact.provenance.path match
        case RefitPath.ExactReduced(algorithm) =>
          assertEquals(algorithm.value, "paired-thin-svd-cross/v1")
        case other =>
          fail(s"unexpected exact path $other")
    }
  }
