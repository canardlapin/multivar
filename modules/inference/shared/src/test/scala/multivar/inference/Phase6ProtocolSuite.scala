...
class Phase6ProtocolSuite extends munit.FunSuite:
  ...
  test("CCA correlation protocol matches independent base-R ridge-CCA roots") {
    ...
    val replicate = accepted(ReplicateId(2))
    val action = accepted(protocol.action(state, replicate, Seed.fromLong(91L)))
  ...
  test("RRR predictive gain uses an explicit held-out split and matches base R") {
    ...
    val protocol = RrrPredictiveProtocol(
      components = acceptedMultivar(ComponentCount(1))
    )
    ...
    val nullA = accepted(protocol.nullStatistic(
      data,
      split,
      Seed.fromLong(12L),
      accepted(ReplicateId(4))
    ))
    val nullB = accepted(protocol.nullStatistic(
      data,
      split,
      Seed.fromLong(12L),
      accepted(ReplicateId(4))
  ...
  test("CPCA block inference matches an independently projected base-R SVD") {
    ...
    val replicate = accepted(ReplicateId(1))
    val action = accepted(protocol.action(state, replicate, Seed.fromLong(55L)))
  ...
  test("multiblock consensus roots match base R and block-independence preserves domains") {
    ...
    val action = accepted(protocol.action(state, replicate, Seed.fromLong(81L)))
