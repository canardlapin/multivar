...
import multivar.core.{ComponentCount, DenseSolvers, MatrixView, SvdSolver}
...
object PrincipalAngles:
  def between(
      left: DMat,
      right: DMat,
      basisSolver: SvdSolver = DenseSolvers.svd,
    ...
    if left.rows != right.rows then
      ...
      for
        qLeft <- orthonormalBasis(left, basisSolver, rankTolerance)
        qRight <- orthonormalBasis(right, basisSolver, rankTolerance)
        cross = InferenceNumerics.transposeMultiply(qLeft, qRight)
        fit <- Svds
          .svd(cross, SingularSelection.All)
          .left
          .map(error =>
            InferenceError.NumericalFailure(
              "principal-angle full SVD",
              error.getMessage
            )
          )
        count = Math.min(qLeft.cols, qRight.cols)
      yield PrincipalAngles(Vector.tabulate(count) { index =>
  ...
  private def orthonormalBasis(
    ...
    if !tolerance.isFinite || tolerance < 0.0 then
      ...
      ComponentCount(Math.min(matrix.rows, matrix.cols))
        .left.map(error =>
          InferenceError.NumericalFailure("subspace component count", error.message)
        )
        .flatMap { components =>
          solver.decompose(MatrixView.dense(matrix), components)
