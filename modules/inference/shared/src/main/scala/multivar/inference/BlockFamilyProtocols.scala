...
import multivar.solver.SpectralRankTolerance
...
final case class CpcaBlockProtocol(
    rankTolerance: SpectralRankTolerance = SpectralRankTolerance.default
      ...
      CpcaInferenceState,
      CpcaBlockFitFamily,
      TargetKind.ConstrainedInertiaRoots,
      NullKind.RowPermutation
    ]:
  type Action = Permutation

  override val fit: FitDescriptor[CpcaBlockFitFamily] = FitDescriptor.CpcaBlock
  ...
  override def action(
    ...
    ResamplingPlans.permutation(
      seed,
      replicate,
      state.problem.rows.descriptor.size
    )
  ...
  override def nullStatistic(
    ...
    for
      ...
      permuted = dense.selectRows(action.toIArray.toIndexedSeq)
      ...
      residual <-
        if blockFit.rank == 0 then
          ...
          ComponentCount(1)
            ...
            .flatMap(one => adapt("CPCA block reconstruction", blockFit.reconstructMetric(Some(one))))
    ...
    CpcaBlockRequest.from(
      ...
      .flatMap(request =>
        state.problem.fit(
          ...
          rankTolerance = rankTolerance.toDouble,
...
final case class MultiblockConsensusProtocol(
    ...
    zeroTolerance: SpectralRankTolerance = SpectralRankTolerance.default
) extends ExactLadderProtocol[
      MultiblockInferenceState,
      MultiblockConsensusFitFamily,
      TargetKind.MultiblockConsensusRoots,
      NullKind.BlockIndependence
    ]:
  type Action = Vector[Permutation]

  override val fit: FitDescriptor[MultiblockConsensusFitFamily] =
  ...
  override val nullHypothesis: NullSpec[NullKind.BlockIndependence] =
  ...
  override def action(
    ...
    ResamplingPlans.multiblockLanes(
      seed,
      replicate,
      state.data.rows,
      Math.max(0, state.partition.blocks.length - 1)
    )
  ...
  override def actionForDesign(
    ...
    for
      ...
      actions <- ResamplingPlans.independent(
        ...
        replicate,
        Math.max(0, state.partition.blocks.length - 1),
        ResamplingPlans.MultiblockDomain
      )
    yield actions
  ...
  override def validateDesign(
    ...
    ProtocolDesign
      .permutation(design, state.data.rows, "multiblock resampling design")
      .map(_ => ())
  ...
  override def nullStatistic(
    ...
    independentlyPermuteBlocks(state, action).flatMap(permuted =>
    ...
    solver.decompose(consensus)
      ...
      .flatMap { eigen =>
        ...
        while i < values.length && error.isEmpty do
          if eigen.values(i) < -zeroTolerance.toDouble then
  ...
  private def independentlyPermuteBlocks(
    ...
    val expected = Math.max(0, state.partition.blocks.length - 1)
    if permutations.length != expected then
      Left(
        InferenceError.RowCountMismatch(
          "multiblock permutation lanes",
          expected,
          permutations.length
        )
      )
      ...
      var blockIndex = 1
      while blockIndex < state.partition.blocks.length do
        val permutation = permutations(blockIndex - 1).toIArray
        ...
        while row < state.data.rows do
          ...
          while local < columns.length do
            ...
            out(row * state.data.cols + col) =
              state.data(permutation(row), col)
            local += 1
          row += 1
        blockIndex += 1
      Right(
