...
final case class CcaCorrelationProtocol(
  ...
  override val fit: FitDescriptor[CcaFitFamily] = FitDescriptor.Cca
  ...
  override def actionForDesign(
    ...
    ProtocolDesign
      .permutation(design, state.y.rows, "CCA resampling design")
  ...
  private def leading(
    ...
    fit(x, y, ridge, 1).map(_.correlations(0))
...
final case class GeneralizedEigenProtocol(
    ...
    zeroTolerance: SpectralRankTolerance = SpectralRankTolerance.default
      ...
      GeneralizedEigenFitFamily,
      TargetKind.GeneralizedEigenRoots,
      NullKind.RowPermutation
    ]:
  type Action = Permutation

  override val fit: FitDescriptor[GeneralizedEigenFitFamily] =
  ...
  override def action(
    ...
    ResamplingPlans.permutation(seed, replicate, state.a.rows)
  ...
  override def actionForDesign(
    ...
    ProtocolDesign
      .permutation(design, state.a.rows, "generalized-eigen resampling design")
      .flatMap(_.draw(seed, replicate))
  ...
  override def validateDesign(
    ...
    ProtocolDesign
      .permutation(
        design,
        state.a.rows,
        "generalized-eigen resampling design"
      )
      .map(_ => ())
  ...
  override def nullStatistic(
    ...
    val permuted =
      FamilyMatrices.symmetricPermutation(
        state.a,
        action.toIArray.toVector
      )
    fit(
      GeneralizedEigenState(permuted, state.b, state.removed),
      1
    ).map(_.roots(0))
  ...
  private[inference] def fit(
    ...
    ComponentCount(state.a.rows)
      .left.map(error =>
        InferenceError.NumericalFailure("generalized eigen component count", error.message)
      )
      .flatMap(requested =>
  ...
  private def validateRoots(
    ...
    while i < values.length do
      ...
      if value < -zeroTolerance.toDouble then
